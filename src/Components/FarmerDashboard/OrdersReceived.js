...
function OrdersReceived() {
  ...
  const handleClick = event => {
    setSelectedCategory(event.target.value === "All Orders" ? null : event.target.value);
  };

  function filteredData(orders, selected, query) {
    return orders.filter(order => {
      const matchesCategory = selected ? order.status.toLowerCase() === selected.toLowerCase() : true;
      const matchesQuery = query ? order.product.toLowerCase().includes(query.toLowerCase()) : true;
  ...
  return (
    <div>
      ...
      <div className='status-flex'>
        <button className='btns' onClick={handleClick} value="All Orders">All Orders</button>
        <button className='btns' onClick={handleClick} value="New Orders">New Orders</button>
